...
export function renderHud(els, state, ui) {
  ...
  const ai = state.players[human.id === 'P1' ? 'P2' : 'P1'];
  const mission = MISSIONS[human.missionId];
  const relic = RELICS[human.relicId];
  const aiRelic = RELICS[ai.relicId];
  ...
  els.topMeta.innerHTML = `
    ...
    <span class="pill ${pid === 'P1' ? 'p1' : 'p2'} ${player.isHuman ? 'turn' : ''}">Turno: ${player.name}</span>
  ...
  els.opponentPanel.innerHTML = `
    <div class="opp-summary">
      <div class="who">
        <strong style="color:var(--p2)">Avversario · IA</strong>
        <span class="pill p2">${countOwned(state, ai.id)} / 42</span>
      </div>
      <div class="stat-row">
        <div class="stat-chip"><span class="k">Armate</span><span class="v">${countArmies(state, ai.id)}</span></div>
        <div class="stat-chip"><span class="k">Bonus</span><span class="v">+${getContinentBonus(state, ai.id)}</span></div>
      </div>
      <div class="relic-mini">
        <div class="name">Reliquia: ${escapeHtml(aiRelic.name)}</div>
      </div>
    ...
    <div class="opp-details">
      <div class="opp-details-inner">
        <div class="relic-mini">
          <div class="desc">${escapeHtml(aiRelic.description)}</div>
        </div>
        <div class="relic-mini">
          <div class="name">Obiettivo: segreto</div>
        </div>
        ${continentPiesHtml(state, ai.id)}
        <div class="hand-backs" title="Carte in mano IA">
          ${ai.hand.map(() => '<div class="card-back"></div>').join('') || '<span style="color:var(--muted);font-size:0.75rem">0 carte</span>'}
        </div>
      </div>
    </div>
  `;
  els.opponentPanel.classList.toggle('is-open', !!ui.opponentExpanded);
  els.opponentPanel.title = ui.opponentExpanded ? 'Clic per ridurre' : 'Clic per espandere';
